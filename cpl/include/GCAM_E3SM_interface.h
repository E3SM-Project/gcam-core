...
public:
    ...
    void runGCAM(int *yyyymmdd, double *gcamoluc, double *gcamoemiss, std::string aBaseLucGcamFileName, std::string aBaseCO2GcamFileName, bool aSpinup,
                 ...
                 int *aNumLon, int *aNumLat, int *aNumPFT, int *aNumReg, int *aNumCty, int *aNumSector, int *aNumPeriod,
                  std::string aMappingFile, int *aFirstCoupledYear, bool aReadScalars, std::string aScalarSourceDir, std::string aELMDataDir, bool aWriteScalars,
    ...
    // Detached (ELM-only -> GCAM-only) ladder support: fill the coupler's ELM arrays from
    // <aDir>/elm2gcam_<aELMYear>_{npp,hr,pft_wt}.csv (gcam/elm_h1_to_gcam.py output, readSpatialDataCSV format).
    static void readELMDataForYear(const std::string& aDir, int aELMYear, int aNumLon, int aNumLat, int aNumPFT,
    ...
    void setLandProductivityScalingGCAM(int *yyyymmdd, double *aELMArea, double *aELMPFTFract, double *aELMNPP, double *aELMHR,
                        ...
                        std::string aELMDataDir, bool aWriteScalars, bool aScaleAgYield, bool aScaleCarbon, std::string aBaseNPPFileName, std::string aBaseHRFileName, std::string aBasePFTWtFileName);
